param(
  [string]$FolderName
)

$ErrorActionPreference = 'Stop'

$repositoryRoot = Split-Path -Parent $PSScriptRoot
$testsRoot = Join-Path $repositoryRoot 'testes'
$manifestPath = Join-Path $testsRoot 'testes.json'

if ($FolderName) {
  $targetRoot = Join-Path $testsRoot $FolderName
  if (-not (Test-Path $targetRoot -PathType Container)) {
    throw "Pasta '$FolderName' não encontrada em '$testsRoot'."
  }

  $title = Split-Path $targetRoot -Leaf
  $indexPath = Join-Path $targetRoot 'index.html'
  $files = Get-ChildItem $targetRoot -File -Filter '*.html' |
    Where-Object { $_.Name -ne 'index.html' } |
    Sort-Object Name

  $cards = foreach ($file in $files) {
    @"
      <a class="card" href="./$($file.Name)">
        <span class="card-title">$($file.Name)</span>
        <span class="card-meta">Arquivo HTML</span>
      </a>
"@
  }

  if ($cards.Count -eq 0) {
    $cards = @"
      <div class="card empty-card">
        <span class="card-title">Nenhum item</span>
        <span class="card-meta">Esta pasta ainda não possui páginas de teste.</span>
      </div>
"@
  }

  $content = @"
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>$title</title>
  <style>
    :root {
      --bg: #0b1220;
      --panel: #111b2d;
      --line: rgba(255,255,255,0.08);
      --text: #e7ecf7;
      --muted: #a6b4cf;
      --primary: #5aa7ff;
      --primary-strong: #2a75d7;
      --shadow: rgba(8, 15, 30, 0.45);
    }

    * { box-sizing: border-box; }

    body {
      margin: 0;
      min-height: 100vh;
      font-family: Arial, Helvetica, sans-serif;
      background: linear-gradient(180deg, #09111b 0%, #0f1a2d 100%);
      color: var(--text);
      display: flex;
      justify-content: center;
      padding: 32px 18px;
      line-height: 1.4;
    }

    .page { width: min(100%, 980px); }

    .topbar {
      display: flex;
      justify-content: flex-start;
      margin-bottom: 20px;
    }

    .back-button {
      display: inline-block;
      text-decoration: none;
      color: var(--text);
      background: rgba(255,255,255,0.04);
      border: 1px solid var(--line);
      border-radius: 10px;
      padding: 10px 16px;
      font-weight: 700;
      transition: transform 0.15s ease, border-color 0.15s ease;
    }

    .back-button:hover,
    .back-button:focus-visible {
      transform: translateY(-1px);
      border-color: rgba(90,167,255,0.8);
      outline: none;
    }

    .header {
      background: linear-gradient(135deg, var(--primary-strong), var(--primary));
      border: 1px solid var(--line);
      border-radius: 18px;
      padding: 28px 24px;
      box-shadow: 0 14px 30px var(--shadow);
      margin-bottom: 24px;
    }

    .eyebrow {
      margin: 0 0 8px;
      text-transform: uppercase;
      letter-spacing: 0.14em;
      font-size: 0.72rem;
      opacity: 0.9;
    }

    h1 {
      margin: 0;
      font-size: clamp(1.8rem, 2.5vw, 2.7rem);
      line-height: 1.15;
    }

    .subtitle {
      margin: 10px 0 0;
      color: rgba(255,255,255,0.85);
      font-size: 0.95rem;
    }

    .grid {
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));
      gap: 18px;
    }

    .card {
      display: block;
      text-decoration: none;
      background: rgba(17,27,45,0.96);
      border: 1px solid var(--line);
      border-radius: 16px;
      padding: 22px 18px;
      min-height: 120px;
      color: var(--text);
      box-shadow: 0 12px 24px rgba(0,0,0,0.18);
      transition: transform 0.18s ease, border-color 0.18s ease, box-shadow 0.18s ease;
    }

    .card:hover,
    .card:focus-visible {
      transform: translateY(-2px);
      border-color: rgba(90,167,255,0.7);
      box-shadow: 0 14px 28px rgba(90,167,255,0.18);
      outline: none;
    }

    .empty-card {
      opacity: 0.75;
      background: rgba(17,27,45,0.7);
    }

    .card-title {
      display: block;
      font-size: 1.08rem;
      font-weight: 700;
      margin-bottom: 6px;
      word-break: break-word;
    }

    .card-meta {
      display: block;
      color: var(--muted);
      font-size: 0.84rem;
    }

    @media (max-width: 560px) {
      .header { padding: 22px 18px; }
      .grid { grid-template-columns: 1fr; }
    }
  </style>
</head>
<body>
  <main class="page">
    <div class="topbar">
      <a class="back-button" href="../index.html">Voltar</a>
    </div>

    <header class="header">
      <p class="eyebrow">SSVP / Testes</p>
      <h1>$title</h1>
      <p class="subtitle">Navegação estática em HTML e CSS, compatível com uso local e com GitHub.</p>
    </header>

    <nav class="grid" aria-label="Itens desta pasta">
$($cards -join "`n")
    </nav>
  </main>
</body>
</html>
"@

  [System.IO.File]::WriteAllText($indexPath, $content, [System.Text.UTF8Encoding]::new($false))
  Write-Output "Índice atualizado: $indexPath"
  exit 0
}

$folders = @{}
Get-ChildItem $testsRoot -Recurse -File -Filter '*.html' |
  Where-Object {
    $_.Name -ne 'index.html' -and
    $_.DirectoryName -ne $testsRoot -and
    $_.DirectoryName -notlike '*\_infra_testes' -and
    $_.DirectoryName -notlike '*\_infra_testes\*'
  } |
  ForEach-Object {
    $relativePath = $_.FullName.Substring($testsRoot.Length + 1).Replace('\', '/')
    $parts = $relativePath.Split('/')
    $fileName = $parts[-1]
    $folder = ($parts[0..($parts.Length - 2)] -join '/')

    if (-not $folders.ContainsKey($folder)) {
      $folders[$folder] = [System.Collections.Generic.List[object]]::new()
    }

    $folders[$folder].Add([ordered]@{ name = $fileName })
  }

$manifest = foreach ($folder in ($folders.Keys | Sort-Object)) {
  [ordered]@{
    folder = $folder
    files = @($folders[$folder] | Sort-Object name)
  }
}

$json = @($manifest) | ConvertTo-Json -Depth 4
[System.IO.File]::WriteAllText($manifestPath, $json + [Environment]::NewLine, [System.Text.UTF8Encoding]::new($false))
Write-Output "Índice gerado: $manifestPath"
